...
// Long term, we should retrieve the current status by asking k8s, the Linode API etc. for all the needed info.
...
func (lc *LinodeClient) instanceStatus(machine *clusterv1.Machine) (instanceStatus, error) {
	if lc.client == nil {
		return nil, nil
	}
	currentMachine, err := util.GetMachineIfExists(lc.client, machine.ObjectMeta.Namespace, machine.ObjectMeta.Name)
	if err != nil {
		return nil, err
	}
	...
	return lc.machineInstanceStatus(currentMachine)
}

// Sets the status of the instance identified by the given machine to the given machine
func (lc *LinodeClient) updateInstanceStatus(machine *clusterv1.Machine) error {
	if lc.client == nil {
		return nil
	}
	...
	m, err := lc.setMachineInstanceStatus(currentMachine, status)
	if err != nil {
		return err
	}

	return lc.client.Update(context.Background(), m)
}

// Gets the state of the instance stored on the given machine CRD
func (lc *LinodeClient) machineInstanceStatus(machine *clusterv1.Machine) (instanceStatus, error) {
	...
	serializer := json.NewSerializer(json.DefaultMetaFactory, lc.scheme, lc.scheme, false)
...
func (lc *LinodeClient) setMachineInstanceStatus(machine *clusterv1.Machine, status instanceStatus) (*clusterv1.Machine, error) {
