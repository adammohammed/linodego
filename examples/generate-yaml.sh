...
Sixth argument must be an S3-style bucket
Seventh argument must be a machine name
Eighth argument must be a pool ID

For example:
./generate-yaml.sh $HOME/.ssh/id_rsa.pub cluster01 $AWS_ACCESS_KEY_ID $AWS_SECRET_ACCESS_KEY $AWS_ENDPOINT $AWS_BUCKET $MACHINE_NAME $POOL_ID'
...
ENCODED_AWS_BUCKET=$(echo -n $6 | base64)
MACHINE_NAME="$7"
POOL_ID="$8"
